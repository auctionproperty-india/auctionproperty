...
if(isset($_POST['activate_sub']) && isset($_POST['sub_id'])) {
    ...
    $start_date = $_POST['start_date']; // Y-m-d
    ...
    $data = $sub->fetch();
    if($data) {
        // Update amount, package_id, and start_date
        $pdo->prepare("UPDATE subscriptions SET amount = ?, package_id = ?, start_date = ? WHERE id = ?")
            ->execute([$amount, $package_id, $start_date, $sub_id]);

        // Calculate end date based on new package
        $pkg_duration = $pdo->prepare("SELECT duration_months FROM packages WHERE id = ?");
        $pkg_duration->execute([$package_id]);
        $duration_months = $pkg_duration->fetchColumn();
        $end = date('Y-m-d', strtotime("$start_date + $duration_months months"));
        $pdo->prepare("UPDATE subscriptions SET status = 'active', end_date = ? WHERE id = ?")->execute([$end, $sub_id]);
        ...
        // ---- Add Income to Accounting with user name and email ----
...
<div class="card-premium">
    ...
    <?php if(isset($_GET['done'])): 
        if($_GET['done'] == 1) echo "<div class='alert alert-success'>✅ Activated with edited details! Income added to accounting.</div>";
