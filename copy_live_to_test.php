...
// ✅ यह Live Database (Render) का सारा Data Test (Neon.tech) पर Copy करेगी
// ⚠️ TEST DATABASE पूरी तरह Overwrite हो जाएगी – पुराना Test Data नहीं बचेगा
// ============================================================
...
// ============================================================
// 🗑️ 1. Test Database को पूरी तरह खाली करें (CASCADE से)
// ============================================================
echo "<h4>🗑️ Clearing Test Database...</h4>";
try {
    $test_pdo->exec("TRUNCATE TABLE 
        users, 
        properties, 
        packages, 
        subscriptions, 
...
try {
    $live_data = $live_pdo->query("SELECT * FROM packages ORDER BY id")->fetchAll(PDO::FETCH_ASSOC);
    $count = 0;
    foreach($live_data as $row) {
        $stmt = $test_pdo->prepare("INSERT INTO packages (id, name, duration_months, price, discount_price, referral_bonus) VALUES (?, ?, ?, ?, ?, ?)");
...
try {
    $live_data = $live_pdo->query("SELECT * FROM properties ORDER BY id")->fetchAll(PDO::FETCH_ASSOC);
    $count = 0;
    foreach($live_data as $row) {
        $stmt = $test_pdo->prepare("INSERT INTO properties (
...
try {
    $live_data = $live_pdo->query("SELECT * FROM subscriptions ORDER BY id")->fetchAll(PDO::FETCH_ASSOC);
    $count = 0;
    foreach($live_data as $row) {
        $stmt = $test_pdo->prepare("INSERT INTO subscriptions (
...
try {
    $live_data = $live_pdo->query("SELECT * FROM user_referral_earnings ORDER BY id")->fetchAll(PDO::FETCH_ASSOC);
    $count = 0;
    foreach($live_data as $row) {
        $stmt = $test_pdo->prepare("INSERT INTO user_referral_earnings (
...
try {
    $live_data = $live_pdo->query("SELECT * FROM account_entries ORDER BY id")->fetchAll(PDO::FETCH_ASSOC);
    $count = 0;
    foreach($live_data as $row) {
        $stmt = $test_pdo->prepare("INSERT INTO account_entries (id, type, amount, description, category, entry_date, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
...
try {
    $live_data = $live_pdo->query("SELECT * FROM wallet_transactions ORDER BY id")->fetchAll(PDO::FETCH_ASSOC);
    $count = 0;
    foreach($live_data as $row) {
        $stmt = $test_pdo->prepare("INSERT INTO wallet_transactions (id, user_id, amount, type, description, reference_id, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->execute([
...
echo "<hr>";
echo "<h3 style='color:green;'>✅ ALL DATA (Users, Properties, Subscriptions, Wallet, Referral, Accounting) COPIED FROM LIVE TO TEST!</h3>";
