...
// Fetch bank details (if table exists, else we'll store in users table or separate)
// For simplicity, we'll add columns to users table: bank_name, account_number, ifsc, branch
// Also need kyc_documents table: id, user_id, doc_type (aadhar, pan, bank_proof), file_path, status, uploaded_at

// Handle POST for personal details update
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['upload_kyc'])) {
    ...
    if(isset($_FILES['kyc_file']) && $_FILES['kyc_file']['error'] == 0) {
        ...
        $filename = 'kyc_' . $user_id . '_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
        move_uploaded_file($_FILES['kyc_file']['tmp_name'], $upload_dir . $filename);
        // Insert into kyc_documents table (create if not exists)
        ...
        <form method="POST">
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="ifsc" class="form-control" value="<?= htmlspecialchars($user['ifsc'] ?? '') ?>">
                </div>
                <div class="col-md-6">
    ...
    <div class="profile-section">
        ...
        <form method="POST" enctype="multipart/form-data">
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    <label class="form-label">Choose File (JPG, PNG, PDF)</label>
                    <input type="file" name="kyc_file" class="form-control" accept=".jpg,.jpeg,.png,.pdf" required>
                </div>
                <div class="col-md-2 d-flex align-items-end">
                    <button type="submit" name="upload_kyc" class="btn btn-success w-100">Upload</button>
                </div>
            </div>
        </form>
        <div class="mt-4">
